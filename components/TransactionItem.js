...
export default function TransactionItem(props) {
  ...
  const handleDelete = () => {
    setIsLoading(true);
    if (user[0]) {
      transactionsRef.doc(props.txDocId).delete();
    }
  };
  ...
  return (
    <div className="flex justify-center mb-8 text-white">
      <div className="font-medium sm:w-8/12 md:w-10/12 w-11/12">
        <p>
          Name{" "}
          <span className="font-light float-right capitalize">
            {props.txName}
          </span>
        </p>
        <p>
        ...
        <p>
          Quantity{" "}
          <span className="font-light float-right">{props.txQuantity}</span>
        </p>
        <p>
          Transaction type{" "}
          <span
            className={`font-light float-right capitalize ${
              props.txType == "buy" ? "text-green-500" : "text-red-500"
            }`}
          >
            {props.txType}
          </span>
        ...
        <p>
          Date <span className="font-light float-right">{date}</span>
        </p>
        <button
          onClick={handleDelete}
          className="w-full bg-lightBlue text-gray-800 rounded-md mt-2 p-1 sm:p-0"
        >
          {isLoading ? (
