  ...
  const handleCoinSearch = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        `https://api.coingecko.com/api/v3/coins/${searchResult}`
      );
      setCoin(data);
      setIsOpen(true);
      setError(false);
  ...
  return (
    <div>
      <form
        ...
        className="mx-auto flex justify-center mt-4 sm:mt-5"
      >
        <input
          className=" bg-darkGray p-2 rounded-sm pl-3 w-64 text-md border-none placeholder-gray-400 border text-white"
          required
          value={searchValue}
        ...
        />
        <button className="p-2 rounded-sm ml-1" type="submit">
          <i aria-hidden className="fas fa-search mr-2 text-lightBlue"></i>
        </button>
      </form>
      {isOpen && (
        <SearchResult
